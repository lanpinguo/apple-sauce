...
/**
 * \defgroup of_list_bsn_tlv of_list_bsn_tlv
 */

/**
 * Create a new of_list_bsn_tlv object
 *
...
of_list_bsn_tlv_new(of_version_t version)
{
    ...
    bytes = of_object_fixed_len[version][OF_LIST_BSN_TLV] + of_object_extra_len[version][OF_LIST_BSN_TLV];

    /* Allocate a maximum-length wire buffer assuming we'll be appending to it. */
    if ((obj = (of_list_bsn_tlv_t *)of_object_new(OF_WIRE_BUFFER_MAX_LENGTH)) == NULL) {
        return NULL;
    }

    of_list_bsn_tlv_init(obj, version, bytes, 0);

    return obj;
}

/**
 * Initialize an object of type of_list_bsn_tlv.
 *
...
void
of_list_bsn_tlv_init(of_list_bsn_tlv_t *obj,
    of_version_t version, int bytes, int clean_wire)
{

    LOCI_ASSERT(of_object_fixed_len[version][OF_LIST_BSN_TLV] >= 0);
    if (clean_wire) {
...
int
of_list_bsn_tlv_next(of_list_bsn_tlv_t *list,
    of_bsn_tlv_t *obj)
{
    int rv;
    ...
    rv = of_object_wire_init((of_object_t *) obj, OF_BSN_TLV,
        list->length);
...
/**
 * Set up to append an object of type of_bsn_tlv to an of_list_bsn_tlv.
...
int
of_list_bsn_tlv_append_bind(of_list_bsn_tlv_t *list,
    of_bsn_tlv_t *obj)
{
...
/**
 * Append an item to a of_list_bsn_tlv list.
 *
...
int
of_list_bsn_tlv_append(of_list_bsn_tlv_t *list,
    of_bsn_tlv_t *item)
{
