...
/**
 * \defgroup of_list_bsn_vlan_counter_stats_entry of_list_bsn_vlan_counter_stats_entry
 */

/**
 * Create a new of_list_bsn_vlan_counter_stats_entry object
 *
...
of_list_bsn_vlan_counter_stats_entry_new(of_version_t version)
{
    ...
    bytes = of_object_fixed_len[version][OF_LIST_BSN_VLAN_COUNTER_STATS_ENTRY] + of_object_extra_len[version][OF_LIST_BSN_VLAN_COUNTER_STATS_ENTRY];

    /* Allocate a maximum-length wire buffer assuming we'll be appending to it. */
    if ((obj = (of_list_bsn_vlan_counter_stats_entry_t *)of_object_new(OF_WIRE_BUFFER_MAX_LENGTH)) == NULL) {
        return NULL;
    }

    of_list_bsn_vlan_counter_stats_entry_init(obj, version, bytes, 0);

    return obj;
}

/**
 * Initialize an object of type of_list_bsn_vlan_counter_stats_entry.
 *
...
void
of_list_bsn_vlan_counter_stats_entry_init(of_list_bsn_vlan_counter_stats_entry_t *obj,
    of_version_t version, int bytes, int clean_wire)
{

    LOCI_ASSERT(of_object_fixed_len[version][OF_LIST_BSN_VLAN_COUNTER_STATS_ENTRY] >= 0);
    if (clean_wire) {
    ...
    obj->version = version;
    obj->length = bytes;
    obj->object_id = OF_LIST_BSN_VLAN_COUNTER_STATS_ENTRY;
...
int
of_list_bsn_vlan_counter_stats_entry_next(of_list_bsn_vlan_counter_stats_entry_t *list,
    of_bsn_vlan_counter_stats_entry_t *obj)
{
    int rv;
    ...
    rv = of_object_wire_init((of_object_t *) obj, OF_BSN_VLAN_COUNTER_STATS_ENTRY,
        list->length);
...
/**
 * Set up to append an object of type of_bsn_vlan_counter_stats_entry to an of_list_bsn_vlan_counter_stats_entry.
...
int
of_list_bsn_vlan_counter_stats_entry_append_bind(of_list_bsn_vlan_counter_stats_entry_t *list,
    of_bsn_vlan_counter_stats_entry_t *obj)
{
...
/**
 * Append an item to a of_list_bsn_vlan_counter_stats_entry list.
 *
...
int
of_list_bsn_vlan_counter_stats_entry_append(of_list_bsn_vlan_counter_stats_entry_t *list,
    of_bsn_vlan_counter_stats_entry_t *item)
{
