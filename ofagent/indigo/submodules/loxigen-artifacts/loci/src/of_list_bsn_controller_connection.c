...
of_list_bsn_controller_connection_new(of_version_t version)
{
    ...
    bytes = of_object_fixed_len[version][OF_LIST_BSN_CONTROLLER_CONNECTION] + of_object_extra_len[version][OF_LIST_BSN_CONTROLLER_CONNECTION];

    /* Allocate a maximum-length wire buffer assuming we'll be appending to it. */
    if ((obj = (of_list_bsn_controller_connection_t *)of_object_new(OF_WIRE_BUFFER_MAX_LENGTH)) == NULL) {
        return NULL;
    }
...
void
of_list_bsn_controller_connection_init(of_list_bsn_controller_connection_t *obj,
...
int
of_list_bsn_controller_connection_first(of_list_bsn_controller_connection_t *list,
    of_bsn_controller_connection_t *obj)
{
    int rv;

    of_bsn_controller_connection_init(obj,
            list->version, 0, 1);
    if ((rv = of_list_first((of_object_t *)list, (of_object_t *)obj)) < 0) {
        return rv;
    }

    of_object_wire_init((of_object_t *) obj, OF_BSN_CONTROLLER_CONNECTION,
                        list->length);
    if (obj->length == 0) {
...
int
of_list_bsn_controller_connection_next(of_list_bsn_controller_connection_t *list,
    of_bsn_controller_connection_t *obj)
{
    int rv;
    ...
    rv = of_object_wire_init((of_object_t *) obj, OF_BSN_CONTROLLER_CONNECTION,
        list->length);
...
int
of_list_bsn_controller_connection_append_bind(of_list_bsn_controller_connection_t *list,
    of_bsn_controller_connection_t *obj)
{
...
/**
 * Append an item to a of_list_bsn_controller_connection list.
 *
...
int
of_list_bsn_controller_connection_append(of_list_bsn_controller_connection_t *list,
    of_bsn_controller_connection_t *item)
{
