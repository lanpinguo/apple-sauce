                        ...
                        int sd)
{
    ...
    *cxn_id = find_free_connection();
    ...
    if (!CXN_LOCAL(cxn)) {
        ...
        cxn->keepalive.pre_threshold = config_params->pre_reset_echo_count;
    } else {
        cxn->keepalive.period_ms = 0;
        cxn->keepalive.threshold = 0;
        cxn->keepalive.pre_threshold = 0;
    }
...
/* Return the config of a specific connection */
indigo_error_t
indigo_cxn_connection_protocol_get(
    indigo_cxn_id_t cxn_id,
    indigo_cxn_protocol_params_t *protocol)
{
    ...
    INDIGO_MEM_COPY(protocol, &connection[cxn_id].protocol_params,
                    sizeof(*protocol));
...
/* Change the config of a specific connection */
indigo_error_t indigo_cxn_connection_config_chg( 
     indigo_cxn_id_t cxn_id,
     indigo_cxn_config_params_t *config)
{
    connection_t *cxn;
    ...
    if (config->local &&
        (config->periodic_echo_ms ||
         config->reset_echo_count)) {
        LOG_ERROR("Local connection with nonzero echo params on cxn add");
        return INDIGO_ERROR_PARAM;
    }
    ...
    INDIGO_MEM_COPY(&cxn->config_params, config, 
                    sizeof(*config));
    
    if (!CXN_LOCAL(cxn)) {
        cxn->keepalive.period_ms = config->periodic_echo_ms;
        cxn->keepalive.threshold = config->reset_echo_count;
        cxn->keepalive.pre_threshold = config->pre_reset_echo_count;
    } else {
        cxn->keepalive.period_ms = 0;
        cxn->keepalive.threshold = 0;
        cxn->keepalive.pre_threshold = 0;
...
indigo_cxn_send_powerdown_message()
{
    ...
    connection_t *cxn;
	of_dpid_t rdpid = 0;
    unsigned char data[50]={0x04,0x0a,0x00,0x32,0x00,0x00,
                  0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x08,
                  0x04,0x00,0xff,0xff,0xff,0xff,0xff,0xff,
                  0xff,0xfe,0x00,0x01,0x00,0x0c,0x80,0x00,
	...
	indigo_core_dpid_get(&rdpid);
    
    INDIGO_MEM_COPY(&data[42], &rdpid, sizeof(of_dpid_t));

    FOREACH_ACTIVE_CXN(cxn_id, cxn) {
        written = write(cxn->sd, data, sizeof(data));
...
void
ind_cxn_stats_show_cli(int details)
{
	aim_log_t* lobj = AIM_LOG_STRUCT_POINTER;
	...
	ind_cxn_stats_show(lobj->pvs, details);
