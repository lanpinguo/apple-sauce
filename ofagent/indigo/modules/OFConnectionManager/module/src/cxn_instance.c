...
cxn_state_set(connection_t *cxn, indigo_cxn_state_t new_state)
{
    ...
    case INDIGO_CXN_S_DISCONNECTED:
        if (cxn->flags & CXN_TO_BE_REMOVED) {
            ...
            /*As tcp server, don't need to retry to connect*/
        }
        else {
...
periodic_keepalive(void *cookie)
{
    ...
    if((cxn->keepalive.pre_threshold != 0) 
      && (cxn->keepalive.outstanding_echo_cnt ==  cxn->keepalive.pre_threshold/*3*/))
    {
        LOG_INFO(cxn, "Exceeded outstanding echo requests pre_threshold. ");
    }
...
ind_cxn_process_write_buffer(connection_t *cxn)
{
    ...
    if (cxn->output_list == NULL) { /* Nothing (more) to send */
        ...
        /*解决cxn->bytes_enqueued ASSERT问题， add by wangjing*/
        if (cxn->bytes_enqueued != 0)
        {
            printf("\r\nind_cxn_process_write_buffer: No more data to write cxn->bytes_enqueued = %d\r\n",cxn->bytes_enqueued);
            cxn->bytes_enqueued = 0;
        }
        if (cxn->pkts_enqueued != 0)
        {
            printf("\r\nind_cxn_process_write_buffer: No more data to write cxn->pkts_enqueued = %d\r\n",cxn->pkts_enqueued);
            cxn->pkts_enqueued = 0;
        }
