                   ...
                   int priority)
{
    indigo_error_t rv;
#ifdef OFDPA_FIXUP
    ind_soc_task_status_t task_status = IND_SOC_TASK_CONTINUE;
#endif
...
#ifdef OFDPA_FIXUP
/*ofdpa删除有顺序要求，把所有操作改成直通*/
    do 
    {
        task_status = ft_iter_task_callback(state);
    }while(task_status == IND_SOC_TASK_CONTINUE);
#else        
