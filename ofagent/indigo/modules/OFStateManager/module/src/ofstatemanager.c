...
ind_core_init(ind_core_config_t *config)
{
    ...
    LOG_INFO("OF state mgr init called");
 ...
 */
void ind_core_flow_expiry_handler(indigo_flow_id_t id,
                                  indigo_fi_flow_removed_t reason)
{
  ft_entry_t *entry = NULL;

  if ((entry = ft_lookup(ind_core_ft, id)) == NULL)
  {
    /* could not find the flow. */
    LOG_ERROR("Could not find the flow id = 0x%x", id);
    return;
  }
